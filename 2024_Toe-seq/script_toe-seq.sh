...
cutadapt -g GACTCGAGTCGACATCGA -a CAGCCGTATGGATTTAACACG -G CGTGTTAAATCCATACGGCTG -A TCGATGTCGACTCGAGTC --report=minimal --match-read-wildcards --trim-n --discard-untrimmed --pair-filter=both -O 7 -m 10 -o trimmed1_R1.fq -p trimmed1_R2.fq $1 $2
cutadapt -g CGTGTTAAATCCATACGGCTG -a TCGATGTCGACTCGAGTC -G GACTCGAGTCGACATCGA -A CAGCCGTATGGATTTAACACG --report=minimal --match-read-wildcards --trim-n --discard-untrimmed --pair-filter=both -O 7 -m 10 -o trimmed2_R1.fq -p trimmed2_R2.fq $1 $2
...
cutadapt -g GCATTAATACGACTCACTATAGGG -e 0.15 --report=minimal -O 13 -m 21 --untrimmed-output wo_T7.fa -o with_T7.fa with_adapt.fa
